...
"""
#         *   `relationship_keywords`: One or more high-level keywords summarizing the overarching nature, concepts, or themes of the relationship. Multiple keywords within this field must be separated by a comma `,`. **DO NOT use `{tuple_delimiter}` for separating multiple keywords within this field.**
#         *   `relationship_keywords`: Categorize the relationship using one of the following types: `{relationship_types}`. If none of the provided relationship types apply, do not add new relationship type and classify it as `Other`.
...
PROMPTS["hierarchical_entity_extraction_system_prompt"] = """---Role---
You are a Knowledge Graph Specialist responsible for extracting entities and relationships from the input text using hierarchical type classification.
...
1.  **Entity Extraction & Output:**
    ...
    *   **Entity Details:** For each identified entity, extract the following information:
        *   `entity_name`: The name of the entity. If the entity name is case-insensitive, capitalize the first letter of each significant word (title case). Ensure **consistent naming** across the entire extraction process.
        *   `entity_type`: Categorize the entity using the most specific appropriate type from: `{entity_types}`. 
            - Use the **most specific** (fine-grained) type when available
            - If a fine-grained type is not available, use the coarse-grained type
